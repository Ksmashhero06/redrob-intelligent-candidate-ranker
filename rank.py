...
def _score_title_fit(profile: dict, history: list) -> float:
    """
    Keyword match in title/description weighted by recency × duration,
    plus shipping-evidence bonus and product company bonus.
    """
    if not history:
        return 0.0
    ...
    weight_sum = 0.0
    shipping_evidence = 0.0
    system_bigram_hits = 0
    ...
    for i, job in enumerate(history):
        ...
        score = min(1.0, title_hits * 0.35 + desc_hits * 0.12)

        weighted_matches += score * role_importance
        weight_sum += role_importance

        # Shipping evidence: real practitioners mention deployment in their descriptions
        ship_hits = sum(1 for k in SHIPPING_KEYWORDS if k in desc)
        if ship_hits > 0:
            shipping_evidence = max(shipping_evidence, min(1.0, ship_hits * 0.15) * recency_w)

        # System bigrams: "built a recommendation system", "deployed search pipeline", etc.
        bigram_hits = sum(1 for bg in SYSTEM_BIGRAMS if bg in desc or bg in title)
        if bigram_hits > 0:
            system_bigram_hits = max(system_bigram_hits, bigram_hits)

        # Product company bonus: not a pure consulting company
        is_service = any(sc in company for sc in SERVICE_COMPANIES)
        if not is_service and i < 3:  # recent roles matter more
            comp_size = job.get("company_size") or ""
            size_score = COMPANY_SIZE_SCORE.get(comp_size, 0.7)
            product_company_bonus = max(product_company_bonus, size_score * recency_w * 0.3)

    title_score = (weighted_matches / weight_sum) if weight_sum > 0 else 0.0

    # Additive bonuses (capped)
    title_score = min(1.0, title_score + shipping_evidence * 0.15 + min(0.1, system_bigram_hits * 0.05) + product_company_bonus * 0.1)

    # Penalize non-technical current titles
    cur_title = (profile.get("current_title") or "").lower()
    if any(k in cur_title for k in NON_TECH_TITLES):
        title_score *= 0.1
...
# ──────────────────────────────────────────────────────────────────────────────
def _score_skill_depth(skills: list, signals: dict) -> float:
    """duration × proficiency × log(1+endorsements), assessment cross-check."""
...
def compute_score(candidate: dict, weights: dict) -> float:
    ...
    skill_score = _score_skill_depth(skills, signals)
...
def generate_reasoning(candidate: dict, score: float) -> str:
    ...
    return (
        ...
        f"{skill_str}{title_ev}{edu_str}{gh_str}; recruiter response rate {response_rate}%."
    )
...
def main():
    ...
    # Sort by rounded score (4dp) so tie-break matches what the validator reads from the CSV
